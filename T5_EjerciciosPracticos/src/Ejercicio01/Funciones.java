...
public class Funciones {
	 ...
	 * @return devuelve el array 'tableroJuego' con el valor de la posición
	 *         'casilla' alterado
	 */
		...
		while (contador < MINAS) { // Bucle while que se cortará tras rellenar 6 minas
			...
			if (tableroMinas[indice] != '*') { //Si hay mina, no pasa nada y se vuelve al while
				...
				if (bordeIzq(indice)) { // Llamamos a la función bordeIzq
					if (!compruebaMinaDer(tableroMinas, indice)) { // Si no hay mina a la derecha, sumamos +1 al 'indice
																	// + 1'
						tableroMinas[indice + 1] += 1;
					}
				} else if (bordeDer(indice)) { // Llamamos a la función bordeDer
					if (!compruebaMinaIzq(tableroMinas, indice)) { // Si no hay mina a la izquierda, sumamos +1 a
																	// 'indice - 1'
						tableroMinas[indice - 1] += 1;
					}
				} else {
					if (!compruebaMinaIzq(tableroMinas, indice)) { // Si no hay mina a la izquierda, sumammos +1 a
																	// 'indice - 1'
						tableroMinas[indice - 1] += 1;
					}
					if (!compruebaMinaDer(tableroMinas, indice)) { // Si no hay mina a la derecha, sumamos +1 a 'indice
																	// + 1'
						tableroMinas[indice + 1] += 1;
					}
				}
